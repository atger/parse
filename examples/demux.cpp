...
{
    if (argc < 4)
        cout << "Usage : " << argv[0] << " <sample_sheet> <fasta file> <allowed mismatch>" << endl;
    else
    {
        ...
        for(auto code : barcodes)
        {
            ofstream f(code.first+".fasta", std::ios::out);
            keys.push_back(code.first);
        }
        ofstream multiclass_file("multiclass.fasta", std::ios::out);
        ofstream unclassified_file("unclassified.fasta", std::ios::out);
        ...
        while(file)
	{
	    ...
	    getline(file,seq);
            ...
            if (!head.empty() || !seq.empty())
            {
		     ...
		     ofstream fasta(keys[ix]+".fasta", std::ios_base::app);
		     fasta << head << endl;
		     fasta << seq << endl;
		     fasta.close();
             ...
             show_progress += (head.length()+seq.length());
	}
    }
    cout << endl;
}
